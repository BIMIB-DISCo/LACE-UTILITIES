...
# Funzione fatta da davide che crea matrice di adiacenza clone - clone
as.adj.matrix.C_C <- function( B ) {
    
    adj_matrix = array(0,c((dim(B)[1]-1),(dim(B)[2]-1)))
    rownames(adj_matrix) <- (2:ncol(B) - 1)
    colnames(adj_matrix) <- (2:ncol(B) - 1)
